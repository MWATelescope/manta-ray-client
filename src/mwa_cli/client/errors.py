...
def parse_api_error(response: httpx.Response) -> APIError:
    ...
    # business logic error
        ...
        return APIError(
            status_code=401, message="Authentication required. Please run: mwa-cli auth login"
        )

    elif status_code == 403:
        return APIError(
            status_code=403, message="Permission denied. You don't have access to this resource."
        )

    elif status_code == 404:
        detail = body.get("detail", "Resource not found")
        return APIError(status_code=404, message=f"Not found: {detail}")
...
def _parse_validation_error(body: dict[str, Any]) -> APIError:
    ...
    if not isinstance(errors, list):
        return APIError(status_code=422, message=f"Validation error: {errors}")
    ...
    for error in errors:
        ...
        field_errors.append({"field": field_name, "message": msg, "type": error.get("type", "")})

        error_messages.append(f"  - {field_name}: {msg}")

    message = "Validation errors:\n" + "\n".join(error_messages)

    return APIError(status_code=422, message=message, field_errors=field_errors)
...
def _parse_business_error(body: dict[str, Any]) -> APIError:
    ...
    return APIError(status_code=400, message=message, error_code=error_code)
