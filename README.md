...
- `clean_iterations: <0 - 1000000>` [optional, default: 100000]
  - corresponds to the `-niter` wsclean parameter
  - Maximum total number of minor iterations (individual clean components) allowed across the entire run.
  - Set this high so your clean stops via thresholds instead of hitting this wall prematurely.
  - Typical: 10,000 - 500,000 depending on field complexity
- `nmiter: <1 - 500>` [optional, default: 10]
  - Maximum number of major iterations (re-gridding loops).
  - For standard fields, you want at least 5 to 10 major loops. This allows WSClean to clean the brightest sources, recalculate the sidelobes accurately, and then clean the next tier of fainter sources.
...
- `mgain <0.6 - 0.95>` [optional. default: default: ] Fraction of flux subtracted per major loop cycle.
- `uvw_min <0.0 - 30.0>` Filters out very short baselines (useful to suppress massive diffuse noise).
- `uvw_max` [optional, default: None] Filters out long baselines. Leave blank to use the array's full resolution.
- `join_polarizations` [optional, default: false] Performs a joint peak-finding operation across linear instrumental polarizations (XX, YY) to ensure accurate primary beam correction later
- `channels_out <1 - 24>` [optional, default: 4] Number of frequency chunks to split the band into for an image cube.
